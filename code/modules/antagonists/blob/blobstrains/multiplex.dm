...
/datum/blobstrain/multiplex/damage_reaction(obj/structure/blob/B, damage, damage_type, damage_flag, coefficient = 1)
	procstart = null
...
/datum/blobstrain/multiplex/death_reaction(obj/structure/blob/B, damage_flag, coefficient = 1)
	procstart = null
	src.procstart = null //when a blob dies, do this
	for (var/datum/blobstrain/bt in blobstrains)
		. += bt.death_reaction(B, damage_flag, coefficient*typeshare)

/datum/blobstrain/multiplex/expand_reaction(obj/structure/blob/B, obj/structure/blob/newB, turf/T, mob/eye/blob/O, coefficient = 1)
	procstart = null
	src.procstart = null //when the blob expands, do this
	for (var/datum/blobstrain/bt in blobstrains)
		. += bt.expand_reaction(B, newB, T, O, coefficient*typeshare)

/datum/blobstrain/multiplex/tesla_reaction(obj/structure/blob/B, power, coefficient = 1)
	procstart = null
...
/datum/blobstrain/multiplex/extinguish_reaction(obj/structure/blob/B, coefficient = 1)
	procstart = null
	src.procstart = null //when the blob is hit with water, do this
	for (var/datum/blobstrain/bt in blobstrains)
		. += bt.extinguish_reaction(B, coefficient*typeshare)

/datum/blobstrain/multiplex/emp_reaction(obj/structure/blob/B, severity, coefficient = 1)
	procstart = null
